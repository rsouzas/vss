	...
	virtual void video(const void * p) {
		if (p) {
			if (emJogo || !mostraSeqImagens) {
				yuyv_to_rgb24(640, 480, (unsigned char*) p,
						(unsigned char*) futCam[indCamera]->pImgCap);
				//memcpy(futCam[indCamera]->pImgCap, p, TAM_IMAGEM_640x480x3);
				if (capturaSeqImagens) {
					if (indSeqImagens >= 0 && indSeqImagens < MAX_SEQ) {
						if (indSeqImagens == 80) {
							btGrava->modify_bg(Gtk::STATE_NORMAL,
									Gdk::Color("red"));
							btGrava->modify_bg(Gtk::STATE_PRELIGHT,
									Gdk::Color("red"));
							btGrava->set_label("@@@@@@@@@@@@@@@@@@@@@@@@");
							printf("?");
						} else {
						...
						memcpy(seqImagens[indSeqImagens++],
								futCam[indCamera]->pImgCap,
								TAM_IMAGEM_640x480x3);
					} else
				...
				printf("%d\n", indSeqImagens);
				if (indSeqImagens >= 0 && indSeqImagens < MAX_SEQ) {
					memcpy(futCam[indCamera]->pImgCap,
							seqImagens[indSeqImagens], TAM_IMAGEM_640x480x3);
				}
			}
			if (!emJogo) {
#ifdef SALVA_ARQ
...
public:
	...
	Gtk::RadioButton rbCorBola, rbCorTime, rbCorContra, rbCorAux1, rbCorAux2,
			rbCorAux3;
	SelecionaCor(int *aHisto, int *bHisto, int *lumHisto) :
			rbCorBola("Bola"), rbCorTime("Time"), rbCorContra("Contra"), rbCorAux1(
					"Aux1"), rbCorAux2("Aux2"), rbCorAux3("Aux3") {
		this->aHisto = aHisto;
		this->bHisto = bHisto;
		...
		add(rbCorAux2);
		add(rbCorAux3);
		rbCorBola.signal_clicked().connect(
				sigc::mem_fun(*this, &SelecionaCor::on_clicked));
		rbCorTime.signal_clicked().connect(
				sigc::mem_fun(*this, &SelecionaCor::on_clicked));
		rbCorContra.signal_clicked().connect(
				sigc::mem_fun(*this, &SelecionaCor::on_clicked));
		rbCorAux1.signal_clicked().connect(
				sigc::mem_fun(*this, &SelecionaCor::on_clicked));
		rbCorAux2.signal_clicked().connect(
				sigc::mem_fun(*this, &SelecionaCor::on_clicked));
		rbCorAux3.signal_clicked().connect(
				sigc::mem_fun(*this, &SelecionaCor::on_clicked));
	}
...
public:
	...
	virtual bool on_expose_event(GdkEventExpose* ev) {
		futCam[indCamEmUso]->imgCap->scale(imgOrig, 0, 0, larg, altu, posX,
				posY, escala, escala, (Gdk::InterpType) 1);
		...
		imgOrig->render_to_drawable(get_window(), get_style()->get_black_gc(),
				0, 0, 0, 0, larg, altu, // draw the whole image (from 0,0 t
		...
		if (win) {
			...
			cr->rectangle(limitesCampo.minXSemGol >> 1, limitesCampo.minY >> 1,
					(limitesCampo.maxXSemGol - limitesCampo.minXSemGol) >> 1,
					(limitesCampo.maxY - limitesCampo.minY) >> 1);
			cr->stroke();
		}
		return true;
	}
	...
	virtual bool on_button_press_event(GdkEventButton *event) {
		if (event->button == 1) { //botao esquerdo
			...
			if ((event->state & GDK_CONTROL_MASK) != 0) { // com control pressionado marca limites do campo
				...
				float fatorX = (futCam[indCamEmUso]->limitesCampo.maxXSemGol
						- futCam[indCamEmUso]->limitesCampo.minXSemGol)
						/ TAM_X_CAMPO_SEM_GOL;
				futCam[indCamEmUso]->limitesCampo.minX =
						futCam[indCamEmUso]->limitesCampo.minXSemGol
								- TAM_X_DO_GOL * fatorX;
				futCam[indCamEmUso]->limitesCampo.maxX =
						futCam[indCamEmUso]->limitesCampo.maxXSemGol
								+ TAM_X_DO_GOL * fatorX;
...
public:
	...
	virtual bool on_expose_event(GdkEventExpose* ev) {
		...
		futCam[indCamEmUso]->imgDigi->render_to_drawable(get_window(),
				get_style()->get_black_gc(), 0, 0, 0, 0, larg, altu, // draw the whole image (from 0,0 t
				Gdk::RGB_DITHER_NONE, 0, 0);
		desenhaElementos();
	...
	int escalaX(float x) {
		LimitesCampo limitesCampo = futCam[indCamEmUso]->limitesCampo;
		float fatorX = (limitesCampo.maxXSemGol - limitesCampo.minXSemGol)
				/ (float) TAM_X_CAMPO_SEM_GOL;
		if (futCam[indCamEmUso]->nossoLado == ESQUERDA)
			return (x - TAM_X_DO_GOL) * fatorX + limitesCampo.minXSemGol;
		else {
			return limitesCampo.minXSemGol
					- (x - (TAM_X_DO_GOL + TAM_X_CAMPO_SEM_GOL)) * fatorX;
		}
	}

	int escalaY(float y) {
		LimitesCampo limitesCampo = futCam[indCamEmUso]->limitesCampo;
		float fatorY = (limitesCampo.maxY - limitesCampo.minY)
				/ (float) TAM_Y_CAMPO;
		if (futCam[indCamEmUso]->nossoLado == ESQUERDA)
	...
	void desenhaElementos() {
		...
		if (win && graficos) {
			...
			for (int i = 0; i < NUM_ROBOS_TIME * 2 + 1; i++) {
				...
				cr->arc(escalaX(estado[i].x), escalaY(estado[i].y), 10, 0,
						2 * M_PI);
				cr->stroke();
			}
			for (int i = 0; i < NUM_ROBOS_TIME; i++) {
				...
				cr->set_line_width(1.0);
				cr->set_source_rgb(1 - (float) i / 3, 0, (float) i / 3);
				cr->move_to(escalaX(objetivoRobo[i].x) + 5,
						escalaY(objetivoRobo[i].y));
				cr->arc(escalaX(objetivoRobo[i].x), escalaY(objetivoRobo[i].y),
						10, 0, 2 * M_PI);
				cr->stroke();

				cr->set_source_rgb(0, 0.8, 0.8);
				cr->move_to(escalaX(objetivoRobo[i].x),
						escalaY(objetivoRobo[i].y));
	...
	virtual bool on_expose_event(GdkEventExpose* event) {
		...
		if (win) {
			...
			for (int indCor = COR_BOLA; indCor < MAX_IND_CORES; indCor++) {
				cr->set_source_rgb(coresRepresenta[indCor].r / 255.0,
						coresRepresenta[indCor].g / 255.0,
						coresRepresenta[indCor].b / 255.0);
...
class JanPrincipal: public Gtk::Window {
	Gtk::Button *btSalva, *btCampoVazio, *btJogo, *btPenalidade, *btPosiciona,
			*btPartida;
	Gtk::VBox vBoxImgOrig, vBoxImgDigi, vBoxControlesCores, vBoxGeral;
	Gtk::HBox hBoxImgs, hBoxSelCor, hBoxCampoVazioLado, hBoxLado, hBoxBotoes,
			hBoxBotoesJogo, hBoxBotoesGrava;
	SelecionaCor *corSel;
...
public:
	JanPrincipal() {
		...
		canalA = new Limites<HistogramaCanalA, BarraCoresCanalA>(aHisto,
				corSel);
		canalB = new Limites<HistogramaCanalB, BarraCoresCanalB>(bHisto,
				corSel);
		canalI = new Limites<HistogramaCanalI, BarraCoresCanalI>(lumHisto,
				corSel);
		...
		btSalva->signal_clicked().connect(
				sigc::mem_fun(*this, &JanPrincipal::btSalvaParams));
		btCampoVazio->signal_clicked().connect(
				sigc::mem_fun(*this, &JanPrincipal::btCapturaCampoVazio));
		nivelVazio->signal_change_value().connect(
				sigc::mem_fun(*this, &JanPrincipal::alterouNivelCampoVazio));
		rbEsquerda->signal_toggled().connect(
				sigc::mem_fun(*this, &JanPrincipal::alterouLado));
		btJogo->signal_clicked().connect(
				sigc::mem_fun(*this, &JanPrincipal::btColocaEmJogo));
		btPenalidade->signal_clicked().connect(
				sigc::mem_fun(*this, &JanPrincipal::btPosicionaPenalidade));
		btPosiciona->signal_clicked().connect(
				sigc::mem_fun(*this, &JanPrincipal::btPosicionaRobos));
		btPartida->signal_clicked().connect(
				sigc::mem_fun(*this, &JanPrincipal::btPreparaPartida));
		tbtGraficos->signal_clicked().connect(
				sigc::mem_fun(*this, &JanPrincipal::tbtApresentaGraficos));

		btVolta->signal_clicked().connect(
				sigc::mem_fun(*this, &JanPrincipal::btGravacaoVolta));
		btGrava->signal_clicked().connect(
				sigc::mem_fun(*this, &JanPrincipal::btGravacaoGrava));
		btAvanca->signal_clicked().connect(
				sigc::mem_fun(*this, &JanPrincipal::btGravacaoAvanca));
	...
	virtual bool on_key_press_event(GdkEventKey *event) {
		if (event->keyval
				== GDK_Escape/* && (event->state & (GDK_SHIFT_MASK | GDK_CONTROL_MASK)) == (GDK_SHIFT_MASK | GDK_CONTROL_MASK)*/)
			emJogo = false;
		return true;
	}
	...
	virtual bool on_expose_event(GdkEventExpose* event) {
		...
		//		cout << "---------------" << endl;
		for (int indCor = COR_BOLA; indCor < MAX_IND_CORES; indCor++) {
			int ax = (futCam[indCamEmUso]->aInicio[indCor]
					+ futCam[indCamEmUso]->aFinal[indCor]) / 2;
			int bx = (futCam[indCamEmUso]->bInicio[indCor]
					+ futCam[indCamEmUso]->bFinal[indCor]) / 2;
			int lum = (int) ((futCam[indCamEmUso]->lumInicio[indCor]
					+ futCam[indCamEmUso]->lumFinal[indCor]) / 2);
			SCT_RGB(ax, bx, lum, coresRepresenta[indCor].r,
					coresRepresenta[indCor].g, coresRepresenta[indCor].b);
			//			show_all_children();
			//			cout << "(" << (int) coresRepresenta[indCor].r << ", " << (int) coresRepresenta[indCor].b << ", " << (int) coresRepresenta[indCor].b << ")" << endl;
		}
		coresRepresenta[MAX_IND_CORES].r = coresRepresenta[MAX_IND_CORES].g =
				coresRepresenta[MAX_IND_CORES].b = 64;
		show_all_children();
...
static gboolean idle(gpointer data) {
	...
	if (cap == NULL) {
		...
		cap = new CapVideo("/dev/video0", 1, (JanPrincipal*) data, 0);
	}
#ifdef VIDEO
	cap->queryFrames();
#else
...
#endif
	futCam[0]->imgDigi = Gdk::Pixbuf::create((Gdk::Colorspace) 0, false, 8, 640,
			480);
